...
"""Candidate onedir spec excluding TensorRT builders and optional Polars."""
...
# Preserve the proven 6-B-1 filter exactly. Refuse to build if the TensorRT
# wheel's builder-resource inventory differs from the validated eight files.
...
# Polars is optional on FitRoute's NumPy/XGBoost inference path. Analysis must
# exclude both its pure modules and separately packaged native runtime; this is
# a guard only, not an additional filter.
