...
"""Candidate onedir spec excluding builders, Polars, and torch/bin/protoc.exe."""
...
# Preserve the validated 6-B-1 builder-resource filter exactly.
...
# Preserve the validated 6-B-2 Polars exclusion and fail closed if it regresses.
unexpected_polars_pure = [
    entry[0]
    for entry in a.pure
    if entry[0] == "polars" or entry[0].startswith("polars.")
]
unexpected_polars_binaries = [
    entry[0]
    for entry in a.binaries
    if Path(entry[0]).name.lower().startswith("_polars_runtime")
    or entry[0].replace("\\", "/").lower().startswith("polars/")
]
if unexpected_polars_pure or unexpected_polars_binaries:
    raise RuntimeError(
        "Polars exclusion was incomplete; refusing candidate build. "
        f"Pure={unexpected_polars_pure}, binaries={unexpected_polars_binaries}"
    )
print("[FitRoute packaging] Polars excluded via Analysis.excludes: PASS")

# 6-B-3B-1: remove only the single, precisely located Torch compiler executable.
expected_protoc_destination = "torch/bin/protoc.exe"
protoc_entries = [
    (collection_name, entry)
    for collection_name, collection in (("binaries", a.binaries), ("datas", a.datas))
    for entry in collection
    if entry[0].replace("\\", "/").lower() == expected_protoc_destination
]
if len(protoc_entries) != 1:
    raise RuntimeError(
        "Torch protoc inventory changed; refusing candidate build. "
        f"Expected protoc.exe entries: 1, actual: {len(protoc_entries)}; "
        f"entries={protoc_entries}"
    )

protoc_collection_name, protoc_entry = protoc_entries[0]
protoc_destination, protoc_source, protoc_typecode = protoc_entry
protoc_source_path = Path(protoc_source).resolve()
if (
    protoc_collection_name != "binaries"
    or protoc_typecode != "BINARY"
    or protoc_source_path.name.lower() != "protoc.exe"
    or protoc_source_path.parent.name.lower() != "bin"
    or protoc_source_path.parent.parent.name.lower() != "torch"
):
    raise RuntimeError(
        "Unexpected Torch protoc.exe source/type; refusing candidate build. "
        f"Collection={protoc_collection_name}, entry={protoc_entry}"
    )

protoc_size = protoc_source_path.stat().st_size
print(
    "[FitRoute packaging] Excluding exactly one Torch compiler executable: "
    f"{protoc_destination} ({protoc_size} bytes, "
    f"{protoc_size / 1024 / 1024:.3f} MiB) <- {protoc_source_path}"
)
a.binaries = [entry for entry in a.binaries if entry != protoc_entry]

remaining_protoc_entries = [
    entry
    for collection in (a.binaries, a.datas)
    for entry in collection
    if entry[0].replace("\\", "/").lower() == expected_protoc_destination
]
if remaining_protoc_entries:
    raise RuntimeError(
        "Torch protoc.exe filter was incomplete; refusing candidate build. "
        f"Remaining={remaining_protoc_entries}"
    )
...
packaged_protoc = [
    path for path in dist_application.rglob("protoc.exe") if path.is_file()
]
if packaged_protoc:
    raise RuntimeError(
        "Torch protoc.exe unexpectedly exists in candidate output: "
        + ", ".join(str(path) for path in packaged_protoc)
    )
print("[FitRoute packaging] Torch protoc.exe absent from candidate output: PASS")
